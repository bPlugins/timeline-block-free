...
const Settings = ({ attributes, setAttributes, activeIndex, setActiveIndex, updateTimeline, isPremium, updateObj }) => {
  ...
  return <>
    <InspectorControls>
      ...
      <TabPanel className='bPlTabPanel' activeClass='activeTab' tabs={generalStyleTabs} onSelect={tabController}>{tab => <>
        ...
        {'general' === tab.name && <General attributes={attributes} setAttributes={setAttributes} addTimeline={addTimeline} duplicateTimeline={duplicateTimeline} updateTimeline={updateTimeline} removeTimeline={removeTimeline} activeIndex={activeIndex} premiumProps={premiumProps} updateObj={updateObj} setActiveIndex={setActiveIndex} />}
    ...
    <AboutProModal isProModalOpen={isProModalOpen} setIsProModalOpen={setIsProModalOpen} link='https://bplugins.com/products/timeline-block/#pricing'>
      <li>&emsp;<strong>{__('Added 6 Beautiful Themes: ', 'custom-html')}</strong>{__('Extend your plugin with these 6 themes. By using this timeline you can create a lot of themes.', 'timeline-block')}</li>
      <li>&emsp;<strong>{__('Horizontal / Vertical Option in every themes: ', 'timeline-block')}</strong>{__("Customize your timeline like horizontal or vertical in some theme as a option. It's easy and beautiful to customize.", 'timeline-block')}</li>
      ...
      <li>&emsp;<strong>{__('Added Powerful Shortcode Timeline Plugin: ', 'timeline-block')}</strong>{__("By this plugin you can use a powerful shortcode timeline plugin with some incredible features. which enabled you to embed the Timeline anywhere in the site.", 'timeline-block')}</li>
    </AboutProModal>
  </>;
};
