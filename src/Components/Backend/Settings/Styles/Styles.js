import { PanelBody, PanelRow, RangeControl, TabPanel, __experimentalUnitControl as UnitControl, __experimentalBorderBoxControl as BorderBoxControl } from '@wordpress/components';
...
const Styles = ({ attributes, setAttributes }) => {
  const { barBackground, barDotColor, dateStyles, itemBg, itemColor, itemTypo, itemBorder, labelTypo, labelColor, iconStyles, theme, contentBorder } = attributes;
  ...
  return (
    <>
      <PanelBody className='bPlPanelBody' title={__('Bar', 'timeline-block')}>
        {
          theme !== "theme-3" && <ColorControl label={__('Background:', 'timeline-block')} value={barBackground} onChange={val => setAttributes({ barBackground: val })} defaultColor='#dddddd' />
        }
...

      {
        theme === "theme-2" && <PanelBody className='bPlPanelBody' title={__('Icon', 'timeline-block')}>
          ...
          <PanelRow>
            <Label className='mb5'>{__("Icon Container Size", "timeline-block")}</Label>
            {/* <Device /> */}
          </PanelRow>
          <RangeControl
      ...
      {
        theme === "theme-4" && <PanelBody className='bPlPanelBody' title={__('Date', 'timeline-block')}>

          <PanelRow>
            <Label className='mb5'>{__("Date Container Size", "timeline-block")}</Label>
            {/* <Device /> */}
          </PanelRow>
          <RangeControl
            value={dateStyles.dateContainerSize}
            onChange={(value) => setAttributes({ dateStyles: updateData(dateStyles, value, "dateContainerSize") })}
            min={5}
            max={100}
          />
          ...
          <Background
            ...
            onChange={val => setAttributes({ dateStyles: updateData(dateStyles, val, "dateBg") })}
            defaults={{ color: '#000' }}
            isImage={false}
          />
      ...
      <PanelBody className='bPlPanelBody' title={__('Item', 'timeline-block')} initialOpen={false}>
        ...
        {
          (theme !== "default" && theme !== "timeline-with-accordion") && <BorderBoxControl
            colors={colors}
            label={__('Border')}
