const iconColor = '#4527a4';
const icons = {
	timeline: <svg xmlns='http://www.w3.org/2000/svg' width={24} height={24} viewBox='0 0 48 48'>
		<rect x='9' y='6' fill='#CFD8DC' width='2' height='36' />
		<circle fill={iconColor} cx='10' cy='10' r='3' />
		<circle fill={iconColor} cx='10' cy='24' r='3' />
		<circle fill={iconColor} cx='10' cy='38' r='3' />
		<path fill='#00BCD4' d='M35,15H20.8c-0.5,0-1-0.2-1.4-0.6l-3.7-3.7c-0.4-0.4-0.4-1,0-1.4l3.7-3.7C19.8,5.2,20.3,5,20.8,5H35 c0.6,0,1,0.4,1,1v8C36,14.6,35.6,15,35,15z' />
		<path fill={iconColor} d='M42,29H20.8c-0.5,0-1-0.2-1.4-0.6l-3.7-3.7c-0.4-0.4-0.4-1,0-1.4l3.7-3.7c0.4-0.4,0.9-0.6,1.4-0.6H42 c0.6,0,1,0.4,1,1v8C43,28.6,42.6,29,42,29z' />
		<path fill='#448AFF' d='M34,43H20.8c-0.5,0-1-0.2-1.4-0.6l-3.7-3.7c-0.4-0.4-0.4-1,0-1.4l3.7-3.7c0.4-0.4,0.9-0.6,1.4-0.6H34 c0.6,0,1,0.4,1,1v8C35,42.6,34.6,43,34,43z' />
	</svg>
};
export default icons;
